...
<input wire:model={{ str_replace(' ','_',$name)}} @readonly($previewmode==1) @required($required==true) @disabled($disabled==true)  id={{str_replace(' ', '-', $name)}} name={{ str_replace(' ','_',$name)}} type={{$type}}
 {!! $attributes->class([
    'w-full dark:bg-gray-900 dark:text-gray-300',
